...
const CustomHero: React.FC<CustomHeroProps> = ({ title, description, buttonText, buttonLink }) => {
  ...
  return (
    <section
      ...
      style={{
        background:
          'linear-gradient(100deg, hsla(214, 64%, 96%, 1) 13%, hsla(214, 30%, 91%, 1) 22%, hsla(215, 32%, 93%, 1) 49%, hsla(216, 100%, 99%, 1) 69%)',
      }}
    >
      <div className="container px-16">
        ...
        <div className="flex flex-col gap-10 md:flex-row md:items-start md:justify-between">
          <div className="max-w-3xl">
            {hasTitle && titleField ? (
              <RichText
                className="bg-linear-to-r from-foreground to-foreground/70 bg-clip-text text-4xl font-bold text-black md:text-4xl lg:text-6xl"
                data={titleField}
                enableGutter={false}
              />
            ) : null}
            {hasButton && buttonLabel && buttonHref && (
              <div className="mt-20 flex flex-col gap-3 sm:flex-row sm:items-center">
                <Button size="lg" className="w-full sm:w-auto bg-black text-[#99f6e4] rounded-xl">
                  <CMSLink url={buttonHref}>{buttonLabel}</CMSLink>
                </Button>
                <CMSLink url={buttonHref}>Explore benefits</CMSLink>
              </div>
            )}
          ...
          <div className="bg-black text-white rounded-xl p-4 shadow-lg mt-48 mr-16 flex items-center gap-2">
