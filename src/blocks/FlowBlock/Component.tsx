              ...
              <div className="flex flex-wrap items-center gap-4">
                {steps.map((step, index) => (
                  <Fragment key={step.title}>
                    <span className="rounded-full bg-background/10 px-5 py-2 text-2xl font-semibold lg:text-4xl">
                      {step.title}
                    </span>
                    {index < steps.length - 1 && (
                      <ArrowRight className="size-10 text-primary-foreground/70" strokeWidth={3} />
                    )}
                  </Fragment>
                ))}
              </div>
